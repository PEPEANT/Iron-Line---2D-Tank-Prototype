...
(function registerInfantryAI(global) {
  ...
  const INFANTRY_CONFIG = {
    ...
    rpgFrontArmorHoldRange: 600,
    rpgVolleyCooldownMin: 2.2,
    rpgVolleyCooldownMax: 2.95,
  ...
  class InfantryAI {
    ...
    update(dt) {
      ...
      if (this.handleRepairAction(dt, repairTarget, beforeX, beforeY)) return;

      if (this.tryUseAiDrone?.(dt, order, contact, tankThreat || reportedVehicleThreat?.target, beforeX, beforeY)) return;

      if (this.handleGrenadeOpportunity(dt, contact, tankThreat)) return;
      ...
      if (this.handleContactEngagement(dt, contact, beforeX, beforeY)) return;

      if (this.handleReportedContact(dt, reportedContact, beforeX, beforeY)) return;

      this.handleAdvanceOrder(dt, order, beforeX, beforeY);
    }

    handleTankThreat(dt, order, contact, tankThreat, beforeX, beforeY) {
      if (tankThreat) {
