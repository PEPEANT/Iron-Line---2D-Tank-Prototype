...
(function bootGame(global) {
  ...
  const ONLINE_WORLD_SYNC_INTERVAL_SECONDS = 0.18;
  ...
  class Game {
    constructor() {
      ...
      this.onlineWorldStateBuffer = [];
      this.onlineWorldStateGapMs = 0;
    ...
    handleLoopError(error) {
      ...
      this.lastLoopErrorMessage = String(error?.message || error || "unknown");
    ...
    updateOnlineWorldSync(dt) {
      ...
      if (this.isOnlineWorldHost(room)) {
        ...
        if (this.onlineWorldSyncTimer <= 0) {
          ...
          this.publishOnlineWorldState(room);
        }
        return;
      }
  ...
  IronLine.installOnlineWorldStateApply?.(Game);
  IronLine.installOnlineWorldStateBuffer?.(Game);
  IronLine.Game = Game;
