    ...
    if (originalEmergencyBailout && !originalEmergencyBailout.__tempoBailoutWrapped) {
      Humvee.prototype.emergencyBailout = function tempoBailoutWrapped(gameArg, options = {}) {
        ...
        const event = {
          ...
          ai: this.ai ? { state: this.ai.state || "", orderRole: this.ai.currentOrder?.role || "", orderStance: this.ai.currentOrder?.stance || "", goal: this.ai.currentOrder?.objectiveName || "", position: { x: Math.round(this.x || 0), y: Math.round(this.y || 0) }, speed: Math.round((Number(options.impactSpeed ?? this.speed) || 0) * 10) / 10, passengers: passengerCountBefore, moveTarget: this.ai.debug?.moveTarget ? { x: Math.round(this.ai.debug.moveTarget.x || 0), y: Math.round(this.ai.debug.moveTarget.y || 0), final: Boolean(this.ai.debug.moveTarget.final), stopDistance: Math.round(Number(this.ai.debug.moveTarget.stopDistance || 0)) } : null, trafficHoldTarget: this.ai.debug?.trafficHoldTarget || "", trafficHoldTimer: Math.round((Number(this.ai.debug?.trafficHoldTimer) || 0) * 10) / 10, stuckTimer: Math.round((Number(this.ai.debug?.stuckTimer) || 0) * 10) / 10, recoveryTimer: Math.round((Number(this.ai.debug?.recoveryTimer) || 0) * 10) / 10, tacticalTrafficHint: this.ai.debug?.tacticalTrafficHint || "", armorThreat: (() => { const item = (gameArg?.tanks || []).filter((tank) => tank?.alive && tank.team !== this.team).map((tank) => ({ tank, distance: Math.hypot((tank.x || 0) - (this.x || 0), (tank.y || 0) - (this.y || 0)) })).sort((a, b) => a.distance - b.distance)[0]; return item ? { id: item.tank.callSign || item.tank.id || "", distance: Math.round(item.distance), los: Boolean(window.IronLine?.physics?.hasLineOfSight?.(gameArg, item.tank, this, { padding: 6 })) } : null; })() } : null
        };
      ...
      for (const unit of infantry) {
        ...
        if (alive) {
          ...
          if (unit.isProne) {
            ...
            proneClassStateCounts[classState] = (proneClassStateCounts[classState] || 0) + 1;
          }
